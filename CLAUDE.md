...
- **SmartDevops no aprobar fixes de seguridad en pg_hba.conf:** El 2026-07-05 SmartDevops propuso y Marce aprobó agregar `host all postgres 0.0.0.0/0 reject` al pg_hba.conf. Esto bloqueó todos los agentes (monitor, smartdevops) que conectan via TCP como `postgres` desde la subnet Docker 172.18.x.x. Fix: remover esas líneas + `SELECT pg_reload_conf()`. Lección: nunca aprobar cambios de pg_hba.conf sin revisar que el Docker subnet siga permitido.
...
- **PM Agent: reconstruido y operativo ✅** (2026-06-13)
  ...
  - **fix_nueva_tarea_parser ✅** (2026-07-05): Prep Nueva regex `#(\w+)` → `#(.+)` + ALIASES map (acepta `#Crypto Agent System`, `#estrategia`, etc.); Insert Task restaurado con prefijo `=` (bug crítico: sin `=` las expresiones `{{ $json }}` eran literales → INSERT 0 0); Fmt Proyectos footer muestra nombres técnicos reales `#crypto_agent · #nodeflow · #depin · #estrategia_b`; help message incluye lista de proyectos
  - **fix_lab_memory_escritura_sistematica ✅** (2026-07-05): PM Agent 71→81 nodos — 10 nodos nuevos (Prep Mem + SSH Write Mem) en paralelo a /nueva, /done, /nuevo_proyecto, tr_approve, tr_reject; SmartDevops 6→8 nodos — Prep Mem SD + SSH Write Mem SD fan-out desde SD Execute Command; registros retroactivos `sesion_5jul_fixes` y `sesion_5jul_comandos` insertados en lab_memory
  - **fix_lab_memory_rag_activo ✅** (2026-07-05): RAG ciclo activo en 4 agentes. Strategy Advisor (7Ohb4fekhWkgfMVE): SSH Ctx Advisor + SSH Ctx Evaluar → query 6 cols (tipo/agente/clave/val/proyecto/fecha) + operativa 24h; Build Advisor Body + Build Eval Body → ctx parsing actualizado a 6 campos. Task Runner (2vlG13sLx4bXAY86): nuevo nodo SSH RAG Context (18 nodos) entre SSH Get File y Build Prompt; Build Prompt incluye `APRENDIZAJES PREVIOS`. PM Agent (HlY3gLWuJowyITB9): nuevo nodo SSH RAG Nueva (82 nodos) entre IF Nueva Valid e Insert Task; Fmt Nueva OK muestra tareas recientes del mismo proyecto. Monkey Brain (uBR0ICIj2ZtLUCvk): Search Similar suma estrategica+aprendizaje (20 registros); Build Research Body label → `MEMORIA ANTERIOR DEL LAB`. SQL: `lab_memory_rag_protocolo` insertado. SmartDevops: `_get_rag_context()` en ClaudeDiagnostics — aprendizajes + sd_operativa 7d + estrategica crypto_agent → sección `APRENDIZAJES LAB MEMORY` en prompt; rebuild + restart confirmado ✅ (commit `302a64b` en VPS).
