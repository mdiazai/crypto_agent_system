...
## Sesión 2026-07-18/19 — [lab][crypto_agent] Spam de alertas "SmartDevops" cada 1 min: emisor real era otro

### Contexto
Marce reportó SmartDevops "roto post-B7": alertas cada 1 min (en vez de cada 30) y falso
positivo permanente del dashboard por chequeo de heartbeat en un servicio pasivo. Se aplicó
un fix completo a `agents/smartdevops/` (HTTP check del dashboard con `DASHBOARD_URL` propio,
cooldown de 1h en `telegram_notifier.py`, deploy verificado end-to-end con container real
caído/restaurado). El spam siguió exactamente igual después del deploy.

### Aislamiento
1. `smartdevops-1` parado 6 minutos completos, confirmado con logs de arranque/parada — spam
   siguió sin cambios.
2. Workflow n8n `11Mkeys SmartDevops Agent` (`qEN2uvjywgpB5jaN`) revisado — 9 nodos, ninguno de
   tipo cron/schedule (solo reactivo a botones `sd_approve`/`sd_ignore`), última ejecución real
   2026-07-16 — descartado.
3. Sin containers duplicados, sin cron ni systemd timer relacionado, sin otro proceso en
   `/opt/crypto_agent_system` referenciando `SMARTDEVOPS_BOT_TOKEN`, sin Docker local corriendo
   en la máquina de Marce.
4. Con los dos sospechosos obvios descartados por evidencia en vivo, se pidió el **texto
   literal** del mensaje en vez de seguir adivinando: `"⚠️ Agentes sin actividad / dashboard /
   Verifica los contenedores."` — formato completamente distinto al de SmartDevops (que manda
   `"🤖 SmartDevops Diagnosis"` con diagnóstico redactado por Claude). Ese dato solo, sin más
   investigación de código, ya apuntaba a un emisor con plantilla fija, no a un LLM.
5. `grep -r` del texto literal en todo `/opt` encontró `orchestrator/agent_supervisor.py` —
   servicio corriendo hace 2+ semanas (puerto 8080), **no documentado en el CLAUDE.md del Lab**.

### Causas raíz (las 3, confirmadas con logs reales, no asumidas)
1. `monitor_loop(interval_seconds=60)` — hardcodeado a 1 minuto en `orchestrator/main.py`.
2. `_check_dashboard()` pegaba a `http://dashboard:8001/health` — ese hostname corto **nunca
   resolvió** en `crypto_agent_network` (confirmado con `socket.gethostbyname` desde dentro del
   container: `gaierror`); el chequeo fallaba siempre por DNS, nunca reflejó el estado real del
   dashboard. Confirmado además que `/health` responde 200 si se le pega al nombre completo del
   container.
3. `_notify_unhealthy()` sin ningún cooldown — reenviaba en cada ciclo de 60s, sin excepción.

### Hallazgo adicional — el bot "equivocado" en realidad no lo era
El container `orchestrator-1` llevaba 2+ semanas sin recrearse (solo restarts, que no releen
`.env` — Lección 8). Su `TELEGRAM_BOT_TOKEN` en memoria seguía siendo el valor de antes de la
separación de bots del 2026-07-08, que coincide numéricamente con el token actual de
`SMARTDEVOPS_BOT_TOKEN` (8141614556). Es decir: el mensaje **realmente salía del bot de
SmartDevops** — Marce no se equivocó de bot, la variable de entorno del orchestrator sí estaba
mal desde antes de esa migración.

### Bug latente adicional descubierto al recrear el container
Al hacer `docker stop+rm+run --env-file` (necesario para picar el `.env` correcto, no un simple
`restart`), el container entró en crash loop: `alembic.util.messaging: Can't locate revision
identified by '0008'`. La imagen Docker de `orchestrator` no se había reconstruido desde antes
del 2026-07-12 (cuando se agregó `0008_add_narrative_tables.py`) — su `alembic/` horneado en la
imagen (`COPY alembic/ ./alembic/` en el Dockerfile, no bind-mount como `orchestrator/` y
`shared/`) no tenía esa revisión, mientras la DB ya estaba en `0008`. Nunca se manifestó porque
el container no se había reiniciado en 2 semanas. Fix: `docker build --no-cache` con el código
actual antes de recrear.

### Fix aplicado
- `shared/config/settings.py`: nuevo campo `dashboard_health_url` (default
  `http://crypto_agent_system-dashboard-1:8001/health`) — fuente única de verdad, usado tanto
  por `orchestrator` como por `smartdevops` (evita que el bug de hostname se repita en un
  tercer lugar futuro).
- `orchestrator/agent_supervisor.py`: `_check_dashboard()` usa `settings.dashboard_health_url`;
  `_notify_unhealthy()` con cooldown de 1h en Redis (hash de agentes ordenados,
  `supervisor:alert_cooldown:*`). Intervalo de 60s se mantiene (chequeo liviano sin LLM,
  decisión de Marce — división de roles: orchestrator detecta caídas rápido, SmartDevops
  diagnostica causas cada 30 min con Claude).
- `agents/smartdevops/health_checker.py`: migrado a `settings.dashboard_health_url` también.
- Imagen `crypto_agent_system-orchestrator` reconstruida con `--no-cache`, container recreado
  con `docker run --env-file` fresco.

### Verificación end-to-end
- 90s post-deploy (1.5 ciclos) sin ninguna alerta, `supervisor:alert_cooldown:*` vacío en Redis.
- Dashboard parado a propósito: alerta real disparó en el primer ciclo (01:50:45 UTC),
  `alert_hash` guardado en Redis confirma el `sendMessage` a Telegram se completó sin error.
  Segundo ciclo con el mismo problema activo (01:51:46 UTC): `alert_suppressed_cooldown`
  registrado, sin reenvío.
- Dashboard restaurado: sin nuevas alertas en los ciclos siguientes.

### Lecciones nuevas agregadas a CLAUDE.md (Lab)
- **21:** el texto literal de un síntoma (no el bot/nombre que se le atribuye) es el dato que
  identifica al emisor real — pedirlo antes de seguir descartando candidatos por código.
- **22:** inventariar `docker ps -a` completo (sin grep por nombre esperado) al investigar
  "qué está mandando esto" — había 15 containers reales, varios sin documentar.
- **23:** `docker restart` no solo no relee `.env` (Lección 8) — si además la imagen no se
  reconstruyó en mucho tiempo, un `stop+rm+run` puede exponer migraciones/código desactualizado
  que nunca se manifestó porque el proceso nunca había vuelto a arrancar limpio.

### Pendiente
- Actualizar inventario de containers y agregar sección "Orchestrator" en CLAUDE.md (Lab) —
  hecho en esta misma sesión, ver arriba.
- Considerar si el fix de SmartDevops (`agents/smartdevops/health_checker.py` +
  `telegram_notifier.py`, sesión previa el mismo día) sigue siendo válido — sí, es un bug real
  separado que también existía, solo que no era la fuente del spam reportado.


---

# Sesiones fusionadas desde crypto_agent_system/Bitacora.md (2026-07-19)

> Las siguientes 37 sesiones (2026-05-13 a 2026-07-04) vivían en `/opt/crypto_agent_system/Bitacora.md`, líneas 829-3569. Se copian acá tal cual, con tag `[crypto_agent]` agregado a cada título, como parte de la reorganización documental B8 (jerarquía de 3 niveles). El archivo original queda intacto — la parte narrativa ("cómo construí esto", líneas 1-828) sigue viviendo solo ahí. Desde esta fecha, las sesiones nuevas de crypto_agent_system se registran directamente acá.

---

## Sesión 2026-05-13 — [crypto_agent] Diagnóstico pipeline + fixes de observabilidad
...
## Sesión 2026-05-13 (turno 2) — [crypto_agent] Fix alertas "Sin alerta" + breakdown de scores
...
## Sesión 2026-05-14 — [crypto_agent] Sincronización CLAUDE.md + endpoint Executor manual
...
## Sesión 2026-05-15 — [crypto_agent] Fix métrica Anticipación Promedio (0.0h → minutos reales)
...
## Sesión 2026-05-15 (turno 2) — [crypto_agent] Max Hold Time + diagnóstico de posiciones abiertas
...
## Sesión 2026-05-16 — [crypto_agent] 4 bugs críticos: stop loss bypass, capital sin límite, large-cap sin filtro
...
## Sesión 2026-05-16 (turno 2) — [crypto_agent] Verificación de containers + fix model ID Claude
...
## Sesión 2026-05-16 (turno 3) — [crypto_agent] Diagnóstico pre-screener + reset circuit breaker
...
## Sesión 2026-05-17 — [crypto_agent] Migración al VPS de Hostinger
...
## Sesión 2026-05-18 — [crypto_agent] Scorer heartbeat + fix GOLD(PAXG) + filtro precio
...
## Sesión 2026-05-19 — [crypto_agent] Multi-chain holder concentration: BSCScan + Helius + Solana
...
## Sesión 2026-05-20 — [crypto_agent] Fix ALERT_THRESHOLD, diagnóstico MAX_HOLD y blacklist USD1/ZEC
...
## Sesión 2026-05-25 al 2026-05-28 — [crypto_agent] Moralis API key truncada + holder_concentration refactor
...
## Sesión 2026-06-06 — [crypto_agent] Crisis de CPU + fix DNS + fix webhook n8n
...
## Sesión 2026-06-07 — [crypto_agent] SmartDevops Agent: build, deploy y primer diagnóstico real
...
## Sesión 2026-06-07 (turno 2) — [crypto_agent] Fix scorer aplanado (detection_score=25 para todos)
...
## Sesión 2026-06-08 — [crypto_agent] Semana 1 Track A: Fix funding pipeline + infraestructura
...
## Sesión 2026-06-13 — [crypto_agent] PM Agent: migración executeCommand → nodos SSH nativos
...
## Sesión 2026-06-18 — [crypto_agent] Incidente: sobreescritura de onchain_client.py por el Code Agent
...
## Sesión 2026-06-20 — [crypto_agent] Protocolo obligatorio Code Agent (post-incidente 18/jun)
...
## Sesión 2026-06-24 — [crypto_agent] /run en PM Agent + eliminación contenedor Python huérfano
...
## Sesión 2026-06-24 (continuación) — [crypto_agent] Tests `/run` con pipes y psql
...
## Sesión 2026-06-25 — [crypto_agent] Blacklist `/run` + Focus Guardian v1
...
## Sesión 2026-06-27 — [crypto_agent] chainid fix en EtherscanClient/BscClient + deploy monitor
...
## Sesión 2026-06-27 (continuación) — [crypto_agent] Weekly Board Agent
...
## Sesión 2026-06-27 (continuación 2) — [crypto_agent] Weekly Board Agent: iteraciones y prueba end-to-end
...
## Sesión 2026-06-27 (continuación 3) — [crypto_agent] Fix chain='unknown' tokens excluidos de holder refresh
...
## Sesión 2026-06-27 (continuación 4) — [crypto_agent] SmartDevops: regla 6b + fix_description
...
## Sesión 2026-06-27 (continuación 5) — [crypto_agent] SmartDevops: fix falso positivo discovery inactivo
...
## Sesión 2026-06-28 — [crypto_agent] Telegram MarkdownV2 fix + verificación SmartDevops
...
## Sesión 2026-06-28 — [crypto_agent] Task Runner + PM Agent Componentes A y C
...
## Sesión 2026-06-29 — [crypto_agent] Fix botones inline Telegram Send Diff (flujo end-to-end completo)
...
## Sesión 2026-07-01 — [crypto_agent] lab_memory + /memoria PM Agent + plan migración DB
...
## Sesión 2026-07-01 (continuación) — [crypto_agent] Migración DB crypto_agent → lab_11mkeys
...
## Sesión 2026-07-03 — [crypto_agent] Strategy Advisor end-to-end + fix webhook 403
...
## Sesión 2026-07-03 — [crypto_agent] Monkey Brain Agent (B1.3)
...
## Sesión 2026-07-04 — [crypto_agent] B2 Evaluación e integración de proyectos
...
## Sesión 2026-07-19 — [lab] B8: Reorganización documental — jerarquía de 3 niveles

### Contexto
Motivado directamente por la sesión anterior (spam del orchestrator): Claude Code trabajando
en `/opt/crypto_agent_system` no conocía el mapa del Lab porque vive en
`/opt/11mkeys_lab/CLAUDE.md`, y el container `orchestrator` corrió 2 semanas sin estar
documentado en ningún lado. Causa raíz: documentación por proyecto con infraestructura
compartida, sin regla de qué vive dónde, con datos duplicados desactualizándose.

### Diagnóstico (Parte 1 — inventario real)
`docker ps -a` completo: 16 containers (no 15 — se sumó `focus_guardian`, que tampoco
figuraba en el inventario anterior). Auditoría de `TELEGRAM_BOT_TOKEN` por container: además
del orchestrator (ya corregido en la sesión previa), se encontraron **3 containers más** con
el token viejo de SmartDevops (8141614556) en su entorno — `monitor-1`, `detector-1`,
`executor-1` — mismo bug de fondo (containers con semanas de uptime, nunca recreados desde
la separación de bots del 2026-07-08). **No se tocaron** — quedan documentados como pendiente
en el maestro, a decisión de Marce, porque no está confirmado que estos tres envíen Telegram
directamente.

10 workflows n8n activos confirmados (sin cambios respecto a la sesión anterior).

Hallazgo adicional: `/opt/crypto_agent_system/CLAUDE.md` resultó ser un **fork abandonado**
del CLAUDE.md del Lab — mismo título, congelado en 2026-07-04, con tokens de bots en texto
plano y webhooks con el patrón viejo (pre-Lección 15). Y `/opt/crypto_agent_system/Bitacora.md`
(3569 líneas) resultó ser dos documentos de género distinto pegados: narrativa de construcción
("cómo construí esto", líneas 1-828) + 37 sesiones en formato log operativo (líneas 829-3569,
2026-05-13 a 2026-07-04) que dejaron de actualizarse cuando el logging se mudó a la Bitácora
del Lab.

### Decisiones de Marce (ante la duda, se preguntó — 3 puntos)
1. La narrativa de `crypto_agent_system/Bitacora.md` queda donde está (documento propio); solo
   las 37 sesiones se copian a la central, con tag `[crypto_agent]`.
2. Los 3 containers con token viejo no se tocan esta sesión — solo se documentan.
3. La nota pendiente "CryptoPanic Growth $179/mes" mencionada en el prompt no se encontró en
   ningún archivo (VPS ni local) — se omite.

### Fix aplicado — jerarquía de 3 niveles
- **Maestro** (`/opt/11mkeys_lab/CLAUDE.md`): nueva sección "INVENTARIO DE INFRAESTRUCTURA"
  con tabla de los 16 containers (función, puerto, uptime, token Telegram, estado) y tabla de
  bots (qué token, quién lo usa, qué canal) — incluye la advertencia de los 3 containers
  pendientes.
- **`crypto_agent_system/CLAUDE.md`**: reemplazado completo (backup en `.pre_b8_backup`) —
  de ~300 líneas con contenido duplicado/obsoleto a 65 líneas con: header estándar "LEER
  PRIMERO" apuntando al maestro, arquitectura de los agentes Python propios del proyecto,
  configuración/thresholds (referencia a `settings.py`, sin duplicar valores), patrón de
  build/deploy específico (containers standalone, Lecciones 8 y 23), y referencia al Narrative
  Swing Module.
- **`narrative_swing/CLAUDE.md`** (repo local de Marce): header reemplazado por el mismo
  formato estándar "LEER PRIMERO" — resto del archivo sin cambios.
- **Bitácora**: las 37 sesiones de `crypto_agent_system/Bitacora.md` (líneas 829-3569) copiadas
  al final de la Bitácora central con tag `[crypto_agent]` en cada título. Archivo original
  intacto (backup en `.pre_b8_backup`) + nota puntero agregada al principio. Entrada de la
  sesión anterior (spam del orchestrator) retitulada con el formato nuevo:
  `[lab][crypto_agent]`.
- **lab_memory**: registro `protocolo_documentacion_3_niveles` (tipo `estrategica`) — ver
  detalle abajo.

### Test del "Claude Code fresco" (verificación final)
Una sesión nueva que arranca en `/opt/crypto_agent_system` y lee su `CLAUDE.md`: se entera en
las primeras 10 líneas de que existe el maestro y que debe leerlo antes de trabajar, del
inventario de 16 containers, y de que hay 3 containers con un bug pendiente sin tocar. Objetivo
cumplido.
